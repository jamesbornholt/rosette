...
(require "ffi.rkt" (only-in "ast.rkt" app assert get-model get-interp get-bool-value)
         "env.rkt" "enc.rkt"
...
(define (encode env solver asserts mins maxs)
  (for ([a asserts])
    (assert (env-ctx env) solver (enc a env)))
  #|
  (for ([m mins])
    (minimize (enc m env)))
  (for ([m maxs])
...
(define (decode env solver)
  (let ([M (get-model (env-ctx env) solver)])
    (sat (for/hash ([(var func) (in-dict (env-decls env))] #:when (constant? var))
           (let ([t (term-type var)])
             (values var (if (function? t)
                             (decode-function env M t func)
                             (decode-value env M t func))))))))

(define (decode-function env model type func)
  (error 'decode-function "NYI"))

(define (decode-value env model type func)
  (let ([interp (get-interp (env-ctx env) model func)])
    (if interp
        (match type
          [(== @boolean?)
           (equal? (get-bool-value (env-ctx env) interp) 'TRUE)])
        (solvable-default type))))
