...
(require "env.rkt" "ffi.rkt" (prefix-in $ "ast.rkt")
...
; a _z3_ast representing that value in the environment. If it
...
(define (enc-expr v env)
  (match v
    ...
    [(expression (app rosette->z3 (? procedure? $op)) es ...) 
     (apply $op (append (list (env-ctx env)) (for/list ([e es]) (enc e env))))]
    [_ (error 'enc "cannot encode ~a to SMT" v)]))

(define (enc-const v env)
  ($app (env-ctx env) (ref! env v)))
