  ...
  d3.flameGraph = function(selector, root, debug) {
    ...
    FlameGraph = (function() {
      ...
      FlameGraph.prototype.size = function(size) {
        ...
        d3.select(this._selector).select('.border-rect').attr('width', this._size[0] - (this._margin.left + this._margin.right)).attr('height', this._size[1] - (this._margin.top + this._margin.bottom));
        return this;
      };
      ...
      FlameGraph.prototype.data = function(root) {
        this.original = d3.flameGraphUtils.augment(root, '0');
        this.root(this.original);
        return this;
      };
      ...
      FlameGraph.prototype._renderNodes = function(containers, attrs, enter) {
        ...
        if (!enter) {
          targetRects = containers.select('rect');
        }
        if (enter) {
        ...
        })(this)).attr('width', attrs.width).attr('height', this.cellHeight()).attr('x', attrs.x).attr('y', attrs.y);
        if (!enter) {
          targetLabels = containers.select('text');
        }
        if (enter) {
          targetLabels = containers.append('text');
        }
        containers.selectAll('text').attr('class', 'label').style('font-size', this.fontSize + "em").attr('dy', (this.fontSize / 2) + "em").attr('x', (function(_this) {
          return function(d) {
            return attrs.x(d) + 2;
          };
