...
(function (profile) {
    ...
    function init() {
        ...
        profile.Profile.sorter = new Tablesort(document.getElementById("profile"), { descending: true });
        renderTable();
    ...
    function renderTable() {
        ...
        profile.Profile.selected.input = getSelectedOption(document.getElementById("input"));
        profile.Profile.selected.output = getSelectedOption(document.getElementById("output"));
        // update the output column
        document.getElementById("output-col").innerHTML = "Avg " + profile.Profile.selected.output;
        // generate the profile entries using these metrics
        var entries = generateProfile(profile.Profile.selected.input, profile.Profile.selected.output);
        ...
        for (var _b = 0, entries_1 = entries; _b < entries_1.length; _b++) {
            ...
            entry_1.row = row;
            profile.Profile.entries.push(entry_1);
            row["entry"] = entry_1;
        ...
        if (profile.Profile.selected.entry != null) {
            for (var _c = 0, _d = profile.Profile.entries; _c < _d.length; _c++) {
                var entry = _d[_c];
                if (entry.name == profile.Profile.selected.entry.name) {
                    new_selection = entry;
                    break;
                }
            }
        }
        if (new_selection == null && profile.Profile.entries.length > 0) {
            new_selection = profile.Profile.entries[0];
        }
        selectEntry(new_selection);
        // sort the table
        profile.Profile.sorter.refresh();
    }
    function renderSubTable(input, output, entry) {
        ...
        callSiteNames.sort();
        // update the output column
        document.getElementById("suboutput-col").innerHTML = "Avg " + profile.Profile.selected.output;
        // update the header
    ...
    function selectEntry(entry) {
        profile.Profile.selected.entry = entry;
        ...
        if (entry != null) {
            ...
            renderGraph(profile.Profile.selected.input, profile.Profile.selected.output, entry);
            renderSubTable(profile.Profile.selected.input, profile.Profile.selected.output, entry);
        }
    }
