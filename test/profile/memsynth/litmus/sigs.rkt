...
(require "lang.rkt" 
         (only-in "../ocelot/ocelot.rkt" 
                  declare-relation make-exact bounds universe))
...
(define (instantiate-test T)
  ...
  (define bMemoryEvent (make-exact MemoryEvent MEs))
  (define bProc
    (make-exact proc (for/list ([a actions]) (list (list-ref ME-atoms  (Action-gid a))
                                                   (list-ref Int-atoms (Action-thd a))))))
  (define bLoc
    (make-exact loc (for/list ([a actions] #:unless (Fence? a)) (list (list-ref ME-atoms (Action-gid a))
                                                  (list-ref Int-atoms (Action-addr a))))))
  (define bData
    (make-exact data (for/list ([a actions] #:unless (Fence? a)) (list (list-ref ME-atoms (Action-gid a))
                                                   (list-ref Int-atoms (Action-val a))))))
  (define bPO
    (make-exact po (for*/list ([a actions][b actions]
  ...
  (define bDP
    (make-exact dp (for*/list ([a actions][b actions]
                               #:when (and (= (Action-thd a) (Action-thd b))
                                            (member (Action-lid a) (Action-deps b))))
                     (list (list-ref ME-atoms (Action-gid a)) (list-ref ME-atoms (Action-gid b))))))
  (define bfinalValue
    (make-exact finalValue (for/list ([AV post])
                             (list (list-ref Int-atoms (car AV)) (list-ref Int-atoms (cdr AV))))))
  (define bRead
    (make-exact Reads (for/list ([a actions] #:when (Read? a)) (list (list-ref ME-atoms (Action-gid a))))))
  (define bWrite
    (make-exact Writes (for/list ([a actions] #:when (Write? a)) (list (list-ref ME-atoms (Action-gid a))))))
  (define bSync
    (make-exact Syncs (for/list ([a actions] #:when (and (Fence? a) (eq? (Fence-type a) 'sync)))
                        (list (list-ref ME-atoms (Action-gid a))))))
  (define bLwsync
    (make-exact Lwsyncs (for/list ([a actions] #:when (and (Fence? a) (eq? (Fence-type a) 'lwsync)))
                          (list (list-ref ME-atoms (Action-gid a))))))
  (define bInt (make-exact Int (for/list ([i Int-atoms]) (list i))))
  (define bZero (make-exact Zero (list (list (first Int-atoms)))))

  (bounds U (list bMemoryEvent bProc bLoc bData bPO bDP bfinalValue bRead bWrite bSync bLwsync bInt bZero)))
