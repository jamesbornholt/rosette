...
(define (make-sketch attrs [bound  (length attrs)])
  ...
  ; A 'bottom' or empty attribute used to 
  ; make sure that every traversal has the same number of attrs.
  ; The checking code just ignores the empty attribute, but having 
  ; all traversals have equal length speeds things up by simplifying
  ; the structure of the sketch value being created.
  (define attrs+empty (cons '(#f #f #f) attrs))
   (for/list ([b bound])
     (cons (choose* #f 'TD 'BU)
           (for/list ([i alen]) (apply choose* attrs+empty)))))

(define (simplify-schedule sched)
  (filter-map
   (lambda (traversal)
     (and (car traversal)
          (cons (car traversal)
           (filter (lambda (attrs) (car attrs)) (cdr traversal)))))
   sched))
