...
(require (for-syntax "../../../rosette/base/core/lift.rkt" racket/syntax)
         (only-in racket/private/generic-methods generic-property)
         (only-in "../../../rosette/base/core/effects.rkt" apply!)
         "../../../rosette/base/core/term.rkt"  "../../../rosette/base/core/lift.rkt" "../../../rosette/base/core/safe.rkt"
         (only-in "../../../rosette/base/core/bool.rkt" || && and-&&)
         (only-in "../../../rosette/base/core/type.rkt" @any/c type-cast gen:typed get-type)
         (only-in "../../../rosette/base/core/procedure.rkt" @procedure?)
         (only-in "../../../rosette/base/core/merge.rkt" merge merge*)
         (only-in "../../../rosette/base/core/union.rkt" union union? in-union-guards)
         (only-in "../../../rosette/base/core/equality.rkt" @equal? @eq?)
         (only-in "../../../rosette/base/adt/generic.rkt" adt-type-cast)
         (only-in "config.rkt" merge-structs?))
...
(struct struct-type (pred super native? make ref set! fields immutable? transparent? procedure? equal+hash)
  ...
  [(define (least-common-supertype type other)
   ...
   (define (type-compress type force? ps) 
     (if (or force? (and (merge-structs?)
                         (struct-type-immutable? type) 
                         (struct-type-transparent? type) 
                         (not (struct-type-equal+hash type))))
